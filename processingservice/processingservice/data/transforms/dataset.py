...
def get_datasets(records: list[InputRecord]) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    df = pd.DataFrame.from_records([pd.json_normalize(r.model_dump(), sep='__') for r in records])
