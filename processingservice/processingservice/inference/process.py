...
def process(payload: Dict[str, Any]) -> bool:
    ...
    try:
        ...
        logger.error(
            f"Failed to load file from S3 for request ID {request_id} : {str(e)}"
        )
    ...
    try:
        ...
        logger.error(
            f"Failed to build input records for request ID {request_id} : {str(e)}"
        )
        update_record_status(request_id, InferenceStatus.FAILED)
        return True
...
def update_record_status(request_id: str, status: InferenceStatus) -> None:
    ...
    update_dynamodb_item(
        table_name="sensorsafrica-requests",
...
def run_inference(
    df_pm_1: pd.DataFrame, df_pm_2_5: pd.DataFrame, df_pm_10: pd.DataFrame
) -> pd.DataFrame:
