...
export class ProcessController {
  ...
  async createProcessRequest(@Req() request: RequestWithUser, @UploadedFile(
    new ParseFilePipe({
      validators: [
    ...
    // Add row to DynamoDB
    await this.fileService.addProcessRequestRow(requestUuid, user.id, objectUrl);
    
    // Add message to SQS
    await this.fileService.sendProcessRequestMessage(requestUuid, user.id, objectUrl);
