...
export class ProcessService {
  ...
  async uploadFile(file: Express.Multer.File, requestUuid: string): Promise<string> {
  ...
  async validateCSVFile(file: Express.Multer.File) {
    ...
    const headers = firstRow.toLowerCase().split(splitCharacter).map(e => e.replace(/['"]+/g, ''));
    const missingHeaders = REQUIRED_CSV_COLUMNS.filter(e => !headers.includes(e));
  ...
  async addProcessRequestRow(requestUuid: string, userId: string, objectUrl: string) {
  ...
  async sendProcessRequestMessage(requestUuid: string, userId: string, objectUrl: string) {
